#1) 
#DATA IMPORT 
newborns=read.csv("newborns.csv", sep = ",", stringsAsFactors = T)  
nrow(newborns)
attach(newborns)
summary(newborns)
library(moments)
...
weight_cl=cut(Peso, breaks = range_weight, labels = weight_labels)
table(weight_cl)

library(ggplot2)
ggplot(data=newborns)+
  geom_bar(aes(x=weight_cl),
           col="black",
           fill="lightblue")+
...
ggplot(newborns, aes(x = factor(Gestazione), y = Peso, fill=Tipo.parto)) +
  geom_boxplot()+
...
x11()
ggplot(data=distr_freq_birth_type) +
...
interaction2=lm(Peso~N.gravidanze+Gestazione+Lunghezza+Cranio+Sesso+Cranio*Lunghezza, data = newborns)
